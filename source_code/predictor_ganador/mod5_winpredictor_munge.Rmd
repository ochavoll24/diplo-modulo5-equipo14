---
title: "Proyecto final módulo 5 - Win Predictor - manipulacion de base de datos"
author: "Alexis_Oscar"
  ...
  full_join(
    ...
    mutate(
      ...
      ,last_elo_band = case_when(
        last_elo <= 1200 ~ 'wood',
        last_elo <= 1400 ~ 'bronze',
        last_elo <= 1600 ~ 'silver',
        last_elo <= 1800 ~ 'gold',
        last_elo <= 2000 ~ 'platinum',
        last_elo <= 2200 ~ 'titanium',
        TRUE ~ 'master'
      )
    ) %>% 
...
chess_munged_db <-
  ...
  mutate(
    opening_class = str_sub(opening_eco,1,1) # clasifica aperturas A-flank, B-semi open, C-Open, D-closed, E-indian defence
    ,base_time = as.numeric(str_trim(before_plus))
    ,add_time = as.numeric(str_trim(after_plus))
    ,total_time = base_time +(add_time* turns/60)
    ,game_class = case_when( #basamos en base time ; summarise(.by = game_class, game_min = min(total_time),game_max = max(total_time)) 
      base_time <3 ~ 'bullet',
      base_time <11 ~ 'blitz',
      base_time <31 ~ 'rapid',
      base_time <181 ~ 'classical',
      TRUE ~ 'other'
    )
    ,rating_dif = white_rating - black_rating
    ,white_elo_band = case_when(
        white_rating <= 1200 ~ 'wood',
        white_rating <= 1400 ~ 'bronze',
        white_rating <= 1600 ~ 'silver',
        white_rating <= 1800 ~ 'gold',
        white_rating <= 2000 ~ 'platinum',
        white_rating <= 2200 ~ 'titanium',
        TRUE ~ 'master'
      )
    ,black_elo_band = case_when(
        black_rating <= 1200 ~ 'wood',
        black_rating <= 1400 ~ 'bronze',
        black_rating <= 1600 ~ 'silver',
        black_rating <= 1800 ~ 'gold',
        black_rating <= 2000 ~ 'platinum',
        black_rating <= 2200 ~ 'titanium',
        TRUE ~ 'master'
      )
